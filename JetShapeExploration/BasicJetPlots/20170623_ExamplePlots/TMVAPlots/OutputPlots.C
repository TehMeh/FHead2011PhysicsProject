...
//	string DataID="PP";
   ...
   TCanvas C;
	...
	for (int i=0;i<MethodNum;i++){
		input=new TFile((inDir+"TMVAOutputAll.root").c_str());
   ...
   TH1F *HROC;//=(TH1F*)File.Get("Method_BDT/BDT_DAB/MVA_BDT_DAB_trainingRejBvsS");
   TH2D *CorrS=(TH2D*)File.Get("CorrelationMatrixS");
   TH2D *CorrB=(TH2D*)File.Get("CorrelationMatrixB");
		...
		for(int i=0; i<VarNum; i++){
			...
			for (int m=a; m<VarMap[VariableName[i]].cycles;m++){
				...
				for(int j=0; j<2;j++){
					...
					H1.SetTitle((Method[k]+" application for "+VariableName[i]+Numbers[m]+";"+rings[m]+AxisName[i]+rings[m+a]).c_str());//because this is the largest histogram it will be the first drawn histogram and provide the title
	...
	for (int k=0; k<MethodNum;k++){
		...
		for(int i=0; i<VarNum;i++){
			...
			for (int m=a; m<VarMap[VariableName[i]].cycles; m++){
				...
				if (VarCounter==0 && MethCounter==0 && (VarOnCount>1 || OnCount>1) && m==a) C.SaveAs((outDir+"QuickPlot.pdf(").c_str());
				else if(MethCounter==OnCount-1 && VarCounter==VarOnCount-1 && VarMap[VariableName[i]].cycles==m+1) C.SaveAs((outDir+"QuickPlot.pdf)").c_str());
				else C.SaveAs((outDir+"QuickPlot.pdf").c_str());
			}//end of rings cycle, m
	   }// end of variable cycle, i

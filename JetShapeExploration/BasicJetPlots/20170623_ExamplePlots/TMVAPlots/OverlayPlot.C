...
{	//Map creation and such
   ...
   TCanvas C;
	...
	TLegend *Legend=new TLegend(0.45, 0.9, 0.9, 0.73, "", "NDC");
	...
	for (int k=0;k<MethodNum;k++){
		...
		for(int i=0; i<VarNum; i++){
			...
			for (int m=a; m<VarMap[VariableName[i]].cycles;m++){
				...
				dataSample.at(indx).SetTitle((Method[k]+", samples, "+VariableName[i]+Numbers[m]).c_str());
				...
				dataLessCut.at(indx).SetTitle((Method[k]+comp[0]+Bound+", Gluon selection, "+VariableName[i]+Numbers[m]).c_str());	
				...
				dataMoreCut.at(indx).SetTitle((Method[k]+comp[1]+Bound+", Quark selection, "+VariableName[i]+Numbers[m]).c_str());
				...
				C.SaveAs((outDir+"ReportOverPlot.pdf(").c_str());
